...
def test_resolve_profile_path_migrates_legacy_dir(profile_paths):
    ...
    resolved = pathing.resolve_profile_path('bot_persistence', allow_legacy=False, migrate_legacy=True)
...
def test_binance_store_migrates_credentials(profile_paths):
    ...
    legacy_file = pathing.PROJECT_ROOT_PATH / 'bot_persistence' / 'binance_credentials.json'
    legacy_file.parent.mkdir(parents=True, exist_ok=True)
    legacy_file.write_text('{"spot": {"api_key": "abc", "api_secret": "def"}}')

    credentials_dir = pathing.resolve_profile_path('credentials', allow_legacy=False, migrate_legacy=True)
    credential_file = os.path.join(credentials_dir, f"{pathing.BOT_PROFILE}.json")

    binance.BinanceCredentialStore(storage_dir=credentials_dir, credential_file=credential_file)
...
def test_profile_paths_isolate_persistence(profile_reloader):
    ...
    alpha_dir = Path(pathing.resolve_profile_path('bot_persistence', allow_legacy=False))
    ...
    bravo_dir = Path(pathing.resolve_profile_path('bot_persistence', allow_legacy=False))
...
def test_binance_store_isolates_multiple_profiles(profile_reloader):
    ...
    alpha_dir = Path(pathing.resolve_profile_path('credentials', allow_legacy=False, migrate_legacy=True))
    alpha_file = os.path.join(alpha_dir, f"{pathing.BOT_PROFILE}.json")
    alpha_store = binance.BinanceCredentialStore(storage_dir=str(alpha_dir), credential_file=alpha_file)
    alpha_store.save_credentials('ALPHA_KEY', 'ALPHA_SECRET', testnet=True, account_type='spot')

    # Switch to tenant bravo and write a different credential payload.
    pathing, binance = profile_reloader('bravo')
    bravo_dir = Path(pathing.resolve_profile_path('credentials', allow_legacy=False, migrate_legacy=True))
    bravo_file = os.path.join(bravo_dir, f"{pathing.BOT_PROFILE}.json")
    bravo_store = binance.BinanceCredentialStore(storage_dir=str(bravo_dir), credential_file=bravo_file)
    bravo_store.save_credentials('BRAVO_KEY', 'BRAVO_SECRET', testnet=False, account_type='futures')
    ...
    reloaded_alpha_store = binance.BinanceCredentialStore(storage_dir=str(alpha_dir), credential_file=alpha_file)
    ...
    reloaded_bravo_store = binance.BinanceCredentialStore(storage_dir=str(bravo_dir), credential_file=bravo_file)
