...
class ComprehensiveTradeHistory:
    ...
    def __init__(
        ...
        *,
        log_callback: Optional[Callable[[str, str, int, Dict[str, Any] | None], None]] = None,
    ) -> None:
        if data_dir is None:
    ...
    # Persistence helpers
    # ------------------------------------------------------------------
    def _log_event(self, message: str, *, level: int = logging.INFO, details: Dict[str, Any] | None = None) -> None:
        if self._log_callback:
            try:
                self._log_callback('TRADE_HISTORY', message, level=level, details=details)
                return
            except Exception:  # pragma: no cover - logging should not break workflow
                logging.getLogger(__name__).exception('Trade history log callback failed')
        ...
        try:
            ...
            for trade in trades:
                if trade['trade_id'] == trade_id and trade['status'] == 'OPEN':
                    ...
                    break
            self.save_trades(trades)
            self._log_event(f'Trade {trade_id} updated with exit data', level=logging.INFO)
            return True
        except Exception as exc:
    ...
    # Analytics
    # ------------------------------------------------------------------
    def get_trade_history(self, filters: Optional[Dict[str, Any]] = None) -> list[Dict[str, Any]]:
        try:
            trades = self.load_trades()
            if filters:
    ...
    def get_trade_statistics(self) -> Dict[str, Any]:
        try:
            ...
            for trade in closed_trades:
                ...
                bucket = strategy_performance.setdefault(strategy, {'trades': 0, 'total_pnl': 0, 'winning_trades': 0})
                bucket['trades'] += 1
            ...
            for trade in closed_trades:
                symbol = trade['symbol']
                bucket = symbol_performance.setdefault(symbol, {'trades': 0, 'total_pnl': 0, 'winning_trades': 0})
                bucket['trades'] += 1
                bucket['total_pnl'] += trade['pnl']
                if trade['pnl'] > 0:
                    bucket['winning_trades'] += 1
            ...
            logging.getLogger(__name__).error('Error calculating trade statistics: %s', exc)
            return self._get_empty_statistics()
        ...
        try:
            ...
            for extra_file in (self.crt_signals_file, self.journal_file):
                if extra_file and os.path.exists(extra_file):
                    ...
                    cleared = True
            if cleared:
                self._log_event('Comprehensive trade history cleared (backup created)', level=logging.INFO)
            return cleared
        except Exception as exc:
    ...
    # Journal support
    # ------------------------------------------------------------------
    def log_journal_event(self, event_type: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        event = {
            'timestamp': datetime.now().isoformat(),
