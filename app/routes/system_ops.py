...
def api_toggle_trading():
    ...
    # Attempt to persist the updated trading flags immediately so the
    # on-disk `bot_state.json` reflects the change.
    try:
        scheduler = _persistence_scheduler(ctx)
        ultimate_ml = ctx.get("ultimate_ml_system")
        trading_config = ctx.get("trading_config") or {}
        ...
        try:
            scheduler.manual_save(
                ultimate, ultimate_ml, trading_config, symbols, history
            )
        except Exception as exc:  # pragma: no cover - best-effort persistence
            print(f"Error saving state after toggle: {exc}")
    except Exception:
