...
def test_futures_trade_records_db(monkeypatch):
    ...
    @pytest.mark.integration
    def test_spot_trade_failure_no_record(monkeypatch):
        monkeypatch.setenv("AI_BOT_TEST_MODE", "1")
        app = create_app()
        ...
        with app.app_context():
            ...
            user = User(username="int_failure", email="intfail@example.com")
            user.set_password("password")
            db.session.add(user)
            db.session.commit()

            # Trader will fail to execute
            app.extensions["ai_bot_context"] = {
                "dashboard_data": {"system_status": {}, "optimized_system_status": {}},
                "get_user_trader": lambda uid, profile: MockUserTrader(success=False),
                "record_user_trade": record_user_trade,
            }
            ...
            resp = client.post("/api/spot/trade", json={"symbol": "BTC", "side": "BUY", "quantity": 0.001})
    ...
    @pytest.mark.integration
    def test_futures_trade_missing_parameters(monkeypatch):
        monkeypatch.setenv("AI_BOT_TEST_MODE", "1")
        app = create_app()
        ...
        with app.app_context():
            ...
            user = User(username="int_missing", email="intmiss@example.com")
            user.set_password("password")
            db.session.add(user)
            db.session.commit()
            ...
            # Missing quantity
            resp = client.post("/api/futures/trade", json={"symbol": "BTCUSDT", "side": "BUY"})
            assert resp.status_code == 400
