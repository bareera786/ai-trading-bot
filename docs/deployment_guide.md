...
2. **Environment variables** (append to `~/.bashrc` or your Compose env file):
   ```bash
...
Logs and health:
- Follow logs: `docker logs -f ai-trading-bot-prod`
- Healthcheck endpoint: `curl http://localhost:5000/health`

Compose also provisions Postgres/Redis; ensure `config/deploy.env` contains your production secrets before starting.
...
The script copies the workspace via `rsync`, compiles the main module on the VPS, and restarts the Docker Compose service. Customize via environment variables:

- `VPS_PORT` (default `22`)
- `COMPOSE_FILE` (default `docker-compose.prod.yml`)
- `DOCKER_SERVICE` (default `ai-trading-bot`)
- `SKIP_RESTART=1` to avoid rebuilding/restarting immediately
...
Docker must be installed on the VPS; the script does not rely on any systemd service for the bot.
