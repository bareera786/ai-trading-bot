...
class RealBinanceTrader:
    ...
    def connect(self):
        ...
        if not self.binance_client_cls:
            ...
            self.logger.error("Binance connect failed - library missing account_type=%s", self.account_type)
            return False
        ...
        if not self.api_key or not self.api_secret:
            ...
            self.connected = False
            self._log_event('CONNECT', self.last_error, severity='error')
            self.logger.error("Binance connect failed - missing credentials account_type=%s", self.account_type)
            return False

        try:
            self.logger.info("Connecting to Binance account_type=%s testnet=%s", self.account_type, self.testnet)
            with self._client_lock:
                self.client = self.binance_client_cls(self.api_key, self.api_secret, testnet=self.testnet)
                if self.testnet:
            ...
            self.logger.info("Binance connect successful account_type=%s testnet=%s", self.account_type, self.testnet)
            return True
        except Exception as exc:
            self.last_error = str(exc)
            self.connected = False
            self._log_event('CONNECT', self.last_error, severity='error')
            self.logger.exception("Binance connect exception account_type=%s", self.account_type)
            return False
    ...
    def set_credentials(self, api_key=None, api_secret=None, auto_connect=True):
        ...
        self.logger.info("Binance credentials updated account_type=%s auto_connect=%s", self.account_type, auto_connect)
        if auto_connect:
            return self.connect()
        return True
    ...
    def set_testnet(self, enabled=True):
        ...
        self.logger.info("Binance testnet toggled account_type=%s testnet=%s", self.account_type, self.testnet)
        if self.client:
            self.connect()
    ...
    def refresh_account_status(self):
        ...
        try:
            ...
            if isinstance(exc, self.api_exception_cls):
                ...
                self.logger.warning("Account status error account_type=%s error=%s", self.account_type, exc)
            else:
                self.last_error = str(exc)
                self._log_event('ACCOUNT_STATUS_ERROR', self.last_error, severity='error')
                self.logger.exception("Account status exception account_type=%s", self.account_type)
        return None
    ...
    def sync_time(self):
        ...
        try:
            ...
            self.logger.debug("Synced Binance server time account_type=%s", self.account_type)
            return server_time
        except Exception as exc:
            self.last_error = str(exc)
            self._log_event('SYNC_TIME_ERROR', self.last_error, severity='error')
            self.logger.warning("Sync time error account_type=%s error=%s", self.account_type, exc)
            return False
    ...
    def _get_symbol_filters(self, symbol):
        ...
        try:
            ...
            self.logger.warning("Unable to fetch symbol filters symbol=%s error=%s", symbol_key, exc)
    ...
    def _normalize_order_quantity(self, symbol, quantity):
        try:
            ...
            self.logger.warning("Failed to normalize quantity symbol=%s qty=%s error=%s", symbol, quantity, exc)
            return float(quantity), None
    ...
    def normalize_price(self, symbol, price):
        ...
        try:
            ...
            self.logger.warning("Failed to normalize price symbol=%s price=%s error=%s", symbol, price, exc)
            return float(price)
    ...
    def get_order_book(self, symbol, limit=5):
        ...
        try:
            ...
            self.logger.warning("Failed to fetch order book symbol=%s error=%s", symbol, exc)
            return None
    ...
    def cancel_order(self, symbol, order_id=None, client_order_id=None):
        ...
        try:
            ...
            self.logger.info("Order cancelled symbol=%s order_id=%s", symbol, order_id or client_order_id)
            return response
        except Exception as exc:
            self.last_error = str(exc)
            severity = 'warning'
            if isinstance(exc, self.api_exception_cls):
                self._log_event('ORDER_CANCEL_FAILED', str(exc), severity=severity, details=params)
                self.logger.warning(
    ...
    def place_real_order(self, symbol, side, quantity, price=None, order_type='MARKET', time_in_force=None):
        ...
        try:
            ...
            self._log_event(
                ...
                severity='success',
                details={'symbol': symbol, 'side': side, 'qty': float(quantity), 'testnet': self.testnet}
            )
            self.logger.info(
    ...
    def _record_order_event(self, status, request, response=None, error=None):
        ...
        if error:
            ...
            self.logger.error("Order event recorded status=%s error=%s symbol=%s", status, error, request.get('symbol'))
        elif status and 'FAIL' in status.upper():
            self._log_event('ORDER_EVENT', f"Order {status}", severity='error', details=request)
            self.logger.error("Order failure recorded status=%s symbol=%s", status, request.get('symbol'))
        return event
...
class BinanceFuturesTrader:
    ...
    def get_market_metrics(self, symbol):
        ...
        try:
            ...
            if isinstance(mark_payload, dict):
                ...
                index_price = self._safe_float(mark_payload.get('indexPrice'), mark_price)
        ...
        try:
            ...
            if isinstance(open_interest_payload, dict):
                open_interest = self._safe_float(open_interest_payload.get('openInterest'))
                if open_interest:
        ...
        try:
            ...
            if isinstance(ticker_payload, dict):
                metrics['taker_buy_volume'] = self._safe_float(ticker_payload.get('takerBuyVolume'))
        except Exception:
            pass
        ...
        try:
            ...
            if isinstance(ratio_payload, list) and ratio_payload:
                metrics['long_short_ratio'] = self._safe_float(ratio_payload[0].get('longShortRatio'), metrics['long_short_ratio'])
        except Exception:
            pass
        ...
        try:
            ...
            if isinstance(position, dict):
                metrics['estimated_liquidation_price'] = self._safe_float(position.get('liquidationPrice'))
        except Exception:
            pass
    ...
    def ensure_leverage(self, symbol, leverage):
        ...
        try:
            ...
            self._log_event(
                ...
                details={'symbol': symbol_key, 'leverage': leverage_int, 'client_type': self._client_type},
            )
            return True
        except Exception as exc:
            self.last_error = str(exc)
    ...
    def place_market_order(self, symbol, side, quantity, reduce_only=False):
        ...
        try:
            ...
            if self.logger:
                self.logger.warning("Futures order failed symbol=%s side=%s error=%s", symbol, side, exc)
            return None
...
def create_user_trader_resolver(
    ...
    *,
    ...
    aliases = {alias.lower() for alias in (optimized_aliases or ('optimized', 'professional', 'pro'))}
