...
class BinanceMarketDataHelper:
    ...
    def _log_rest_failure(self, message: str, severity: str = "error") -> None:
        if not self._binance_log_manager:
            return
        try:
            self._binance_log_manager.add(
                "REST_API_ERROR", message, severity=severity, account_type="spot"
            )
    ...
    def fetch_24hr_ticker(
        self, symbol: str | None = None, timeout: float = 10.0
    ) -> Any:
        ...
        for base_url in self._resolve_rest_hosts():
            try:
                ...
                self.logger.debug(
                    "Requesting Binance ticker host=%s symbol=%s",
                    base_url,
                    symbol or "ALL",
                )
                ...
                if response.status_code == 200:
                    self.logger.debug(
                        "Binance ticker success host=%s symbol=%s",
                        base_url,
                        symbol or "ALL",
                    )
                ...
                log_fn = (
                    self.logger.warning
                    if self._should_emit_warning(warn_key)
                ...
                last_error = exc
                warn_key = (
                    f"exception|{base_url}|{symbol or 'ALL'}|{type(exc).__name__}"
                )
                log_fn = (
                    self.logger.warning
                    if self._should_emit_warning(warn_key)
        ...
        if last_error:
            self.logger.error(
                "Binance ticker failed after all hosts symbol=%s error=%s",
                symbol or "ALL",
                last_error,
            )
        ...
        try:
            ...
            if isinstance(all_data, list):
                usdt_pairs = [
                    ...
                    if str(pair.get("symbol", "")).endswith("USDT")
                ]
                trending = sorted(
                    usdt_pairs,
                    key=lambda x: self._safe_float(x.get("volume")),
                    reverse=True,
                )[:5]
                trending_data = [
                    {
                ...
                self.logger.info(
                    "Trending pairs refreshed count=%d", len(trending_data)
                )
    ...
    def get_real_market_data(self, symbol: str) -> dict[str, Any]:
        try:
            ...
            self.logger.error(
                "Market data fetch failed symbol=%s", symbol, exc_info=True
            )
