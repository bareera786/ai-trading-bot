...
$SSH_CMD "sudo chown -R aibot: $VPS_PATH" || true
# chown common data directories so the image user (appuser, UID/GID ${CONTAINER_UID}) can write
$SSH_CMD "set -a && . $VPS_PATH/config/deploy.env.production && for p in bot_persistence futures_models optimized_models ultimate_models trade_data app/static app/templates api_routes; do if [ -d $VPS_PATH/\$p ]; then sudo chown -R aibot: $VPS_PATH/\$p && sudo chmod -R u+rwX,g+rwX $VPS_PATH/\$p; fi; done" || true"
...
$SSH_CMD "mkdir -p $VPS_PATH/reports && if [ ! -f $VPS_PATH/reports/backtest_top10.json ]; then echo '{\"generated_at\": null, \"aggregate_summary\": {}, \"symbol_summaries\": {}}' > $VPS_PATH/reports/backtest_top10.json; fi && sudo chown -R aibot: $VPS_PATH/reports && sudo chmod -R u+rwX,g+rwX $VPS_PATH/reports" || true
echo ""
