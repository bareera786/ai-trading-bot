...
class StrategyManager:
    ...
    def update_ml_feedback(self, strategy_name, trade_result, qfm_features=None):
        ...
        self.ml_feedback['parameter_history'].setdefault(strategy_name, []).append(param_entry)
    ...
    def _run_scheduled_optimization(self):
        ...
        schedule['next_optimization'] = current_time + (schedule['frequency_hours'] * 3600)
    ...
    def _bayesian_parameter_optimization(self, strategy_name, param_ranges, performance_data):
        ...
        for _ in range(n_samples):
            params = {param: random.uniform(*bounds) for param, bounds in param_ranges.items()}
    ...
    def _update_performance_monitoring(self):
        ...
        if len(monitoring['performance_history']) > 1000:
            monitoring['performance_history'] = monitoring['performance_history'][-1000:]
    ...
    def _should_retrain_models(self):
        ...
        new_samples = len([p for p in getattr(self, 'performance_history', []) if p['timestamp'] > last_retrain])
    ...
    def get_continuous_improvement_status(self):
        ...
        return {
            ...
            'performance_monitoring': {
                ...
                'current_metrics': self.calculate_overall_performance(30) if getattr(self, 'performance_history', None) else {},
            },
            'automated_optimization': {
                'enabled': optimization['enabled'],
                'method': optimization['optimization_method'],
                'last_improvement': optimization['optimization_history'][-1] if optimization['optimization_history'] else None,
            },
            'model_retraining': {
            ...
            'system_health': {
                ...
                'current_status': health['health_history'][-1] if health['health_history'] else None,
    ...
    def calculate_adaptive_position_size(self, symbol, base_position_size, market_data=None, strategy_name=None):
        ...
        if self.qfm_engine:
            ...
            qfm_features = self.qfm_engine.compute_realtime_features(symbol, latest_candle)
    ...
    def _classify_market_regime(self, qfm_features):
        ...
        if trend_confidence > 0.7 and velocity > 0.3:
            return 'trending_bull' if qfm_features.get('velocity', 0) > 0 else 'trending_bear'
    ...
    def _get_strategy_risk_adjustment(self, strategy_name, regime, volatility_level):
        ...
        strategy_adjustments = {
            ...
            'trend_following': {'trending_bull': 1.3, 'trending_bear': 1.3, 'volatile': 0.7},
            'mean_reversion': {'sideways': 1.2, 'volatile': 0.6, 'trending_bull': 0.8, 'trending_bear': 0.8},
    ...
    def calculate_overall_performance(self, window_days=30):
        ...
        recent_returns = [entry.get('returns', 0) for entry in self.performance_history[-window_days * 10:]]
        if recent_returns:
            mean_return = np.mean(recent_returns)
            std_return = np.std(recent_returns) if len(recent_returns) > 1 else 0.0
            sharpe_ratio = (mean_return / std_return) * np.sqrt(252) if std_return else 0.0
        else:
            sharpe_ratio = 0.0

        wins_pnl = sum(max(0, r) for r in recent_returns)
        losses_pnl = abs(sum(min(0, r) for r in recent_returns))
        profit_factor = (wins_pnl / losses_pnl) if losses_pnl else (wins_pnl if wins_pnl else 0.0)
    ...
    def execute_strategy(self, strategy_name, symbol, market_data=None, indicators=None):
    ...
    def _simulate_backtest_results(self, job_payload):
        ...
        for strategy_name in strategies:
            ...
            simulated.append({
                ...
                'total_return_pct': performance.get('total_pnl', 0) * random.uniform(0.8, 1.2),
        ...
        try:
            for strategy_name in self.strategies:
                if not self.active_strategies.get(strategy_name, True):
                    continue
                results[strategy_name] = self.optimize_strategy_parameters(strategy_name)
        except Exception as exc:  # pylint: disable=broad-except
            ...
            self.optimization_status['recent_results'] = self.optimization_status['recent_results'][-5:]
