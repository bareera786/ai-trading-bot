...
def _ml_system(ctx: dict[str, Any], mode: str):
    system = ctx.get('optimized_ml_system') if mode == 'optimized' else ctx.get('ultimate_ml_system')
    if not system:
...
def api_performance_chart():
    ...
    chart = dashboard_data.get('performance_chart') or dashboard_data.get('performance') or {}
...
def api_persistence_backups():
    ...
    if backup_dir and os.path.exists(backup_dir):
        for filename in os.listdir(backup_dir):
            if not (filename.startswith('state_backup_') and filename.endswith('.json')):
                continue
            file_path = os.path.join(backup_dir, filename)
            created = datetime.fromtimestamp(os.path.getctime(file_path)).isoformat()
            size_kb = os.path.getsize(file_path) // 1024
            backups.append({'filename': filename, 'created': created, 'size_kb': size_kb})
...
def api_toggle_trading():
    ...
    system_status['real_trading_ready'] = bool(getattr(ultimate, 'real_trading_enabled', False))

    optimized_status = dashboard_data.setdefault('optimized_system_status', {})
    optimized_status['trading_enabled'] = enable if optimized else optimized_status.get('trading_enabled')
    optimized_status['paper_trading'] = getattr(optimized, 'paper_trading', True) if optimized else optimized_status.get('paper_trading')
    optimized_status['real_trading_ready'] = bool(getattr(optimized, 'real_trading_enabled', False)) if optimized else optimized_status.get('real_trading_ready')
...
def api_disable_symbol():
    ...
    ultimate_system = ctx.get('ultimate_ml_system')
    optimized_system = ctx.get('optimized_ml_system')
    disabled_ultimate = ultimate_system.remove_symbol(symbol, permanent=False) if ultimate_system else False
    disabled_optimized = optimized_system.remove_symbol(symbol, permanent=False) if optimized_system else False

    if not disabled_ultimate and not disabled_optimized and not _is_symbol_disabled(ctx, symbol):
    ...
    return jsonify({'message': f'Symbol {symbol} disabled successfully', 'symbol': symbol, 'status': 'disabled'})
...
def api_list_symbols():
    ...
    active_set = set(get_universe()) if callable(get_universe) else set(sym for sym in _top_symbols(ctx))
    disabled_set = set(get_disabled()) if callable(get_disabled) else set(_disabled_symbols(ctx))
