    ...
    try:
        ...
        for portfolio in all_portfolios:
            ...
            for symbol in major_symbols:
                symbol_exposure[symbol] = symbol_exposure.get(symbol, 0) + exposure_per_symbol
...
def api_admin_performance_benchmarking():
    ...
    try:
        ...
        benchmarks = {
            ...
            'historical_performance': {'system_performance': {}, 'benchmark_comparison': {}, 'rolling_performance': {}},
            'peer_comparison': {'top_performers': [], 'average_performance': {}, 'performance_distribution': {}},
        ...
        if total_investment > 0:
            ...
            oldest_trade = min((t.timestamp for t in all_trades if t.timestamp), default=datetime.now())
...
def api_admin_automated_strategy_optimization():
    ...
    try:
        ...
        optimization = {
            'current_system_analysis': {'overall_performance': {}, 'strategy_effectiveness': {}, 'risk_exposure': {}, 'market_conditions': {}},
        ...
        for trade in all_trades:
            ...
            stats = strategy_performance.setdefault(strategy, {'trades': 0, 'wins': 0, 'pnl': 0.0, 'avg_pnl': 0.0})
            stats['trades'] += 1
        ...
        recent_win_rate = (
            len([t for t in recent_trades if (t.pnl or 0) > 0]) / len(recent_trades) * 100 if recent_trades else 0
        )
...
def api_admin_user_activity_monitoring():
    ...
    try:
        ...
        activity_report = {
            ...
            'user_engagement': {'login_patterns': {}, 'trading_activity': {}, 'engagement_scores': {}, 'user_segments': {}},
            'activity_metrics': {
                'daily_active_users': {},
                'weekly_active_users': {},
            ...
            'retention_analysis': {'user_retention': {}, 'churn_risk': {}, 'engagement_trends': {}},
            'timestamp': time.time(),
        }
        ...
        activity_report['user_engagement']['user_segments'] = {
            'high_engagement': {
                ...
                'avg_trades': sum(u['trade_count'] for u in high) / len(high) if high else 0,
            },
            'medium_engagement': {
                'count': len(medium),
                'percentage': (len(medium) / total_users) * 100 if total_users > 0 else 0,
                'avg_trades': sum(u['trade_count'] for u in medium) / len(medium) if medium else 0,
            },
            'low_engagement': {
                'count': len(low),
                'percentage': (len(low) / total_users) * 100 if total_users > 0 else 0,
                'avg_trades': sum(u['trade_count'] for u in low) / len(low) if low else 0,
            },
        }

        new_users_30 = len([u for u in all_users if u.created_at and (datetime.now() - u.created_at).days <= 30])
        active_last_30 = len([
            u for u in user_activity.values()
            if u['last_login'] and (datetime.now() - datetime.fromisoformat(u['last_login'])).days <= 30
        ])
        activity_report['retention_analysis']['user_retention'] = {
            'new_users_30_days': new_users_30,
            'active_users_30_days': active_last_30,
            'retention_rate_30_days': (active_last_30 / total_users) * 100 if total_users > 0 else 0,
            'churn_rate_estimated': ((total_users - active_last_30) / total_users) * 100 if total_users > 0 else 0,
        }
...
def api_admin_consolidated_pnl():
    ...
    try:
        ...
        for trade in all_trades:
            strategy = trade.signal_source or 'unknown'
            stats = strategy_stats.setdefault(strategy, {'trades': 0, 'pnl': 0.0, 'wins': 0, 'volume': 0.0, 'users': set()})
            stats['trades'] += 1
            stats['pnl'] += trade.pnl or 0
        ...
        for strategy, stats in strategy_stats.items():
            report['breakdowns']['by_strategy'][strategy] = {
                ...
                'user_adoption_rate': len(stats['users']) / report['summary']['total_users'] * 100 if report['summary']['total_users'] > 0 else 0,
            }

        symbol_stats = {}
        for trade in all_trades:
            symbol = trade.symbol or 'UNKNOWN'
            stats = symbol_stats.setdefault(symbol, {'trades': 0, 'pnl': 0.0, 'wins': 0, 'volume': 0.0, 'users': set()})
            stats['trades'] += 1
            stats['pnl'] += trade.pnl or 0
            stats['volume'] += (trade.quantity or 0) * (trade.entry_price or 0)
            stats['users'].add(trade.user_id)
        ...
        user_stats = {}
        for trade in all_trades:
            stats = user_stats.setdefault(trade.user_id, {
                'trades': 0,
                'pnl': 0.0,
                'wins': 0,
                'volume': 0.0,
        ...
        for user_id, stats in user_stats.items():
            ...
            report['breakdowns']['by_user'][username] = {
                ...
                'activity_score': stats['trades'] / report['summary']['total_trades'] * 100 if report['summary']['total_trades'] > 0 else 0,
            }
        ...
        for trade in all_trades:
            ...
            if risk_percentage <= 1:
                ...
                risk_level = 'extreme'
            stats = report['breakdowns']['by_risk_level'].setdefault(risk_level, {
                'trades': 0,
                'pnl': 0.0,
                'wins': 0,
                'avg_risk': 0.0,
        ...
        for trade in all_trades:
            ...
            stats = trade_types.setdefault(trade_type, {'trades': 0, 'pnl': 0.0, 'wins': 0, 'volume': 0.0})
            stats['trades'] += 1
            stats['pnl'] += trade.pnl or 0
            stats['volume'] += (trade.quantity or 0) * (trade.entry_price or 0)
            if (trade.pnl or 0) > 0:
        ...
        for trade_type, stats in trade_types.items():
            report['breakdowns']['by_trade_type'][trade_type] = {
                ...
                'avg_pnl': stats['pnl'] / stats['trades'] if stats['trades'] > 0 else 0,
                'volume': stats['volume'],
                'percentage': stats['trades'] / report['summary']['total_trades'] * 100 if report['summary']['total_trades'] > 0 else 0,
            }
...
def api_admin_dashboard():
    ...
    try:
        ...
        symbol_exposure = {}
        for portfolio in all_portfolios:
            current_value = portfolio.quantity * (portfolio.current_price or portfolio.avg_price) if hasattr(portfolio, 'quantity') else 0
        ...
        return jsonify({
            ...
            'strategy_performance': strategy_manager.get_all_performance() if strategy_manager else {},
            'timestamp': time.time(),
        })
    except Exception as exc:  # pragma: no cover - defensive logging
