...
def api_journal():
    ...
    if mode == "optimized":
        ...
        events = _annotated_events(ultimate, "ultimate", per_limit) + _annotated_events(
            optimized, "optimized", per_limit
        )
...
def api_journal_filters():
    ...
    symbols = (
        set(str(symbol).upper() for symbol in (get_universe() or []))
        if get_universe
        else set()
    )
    event_types = set()
    ...
    if (not results) or all(not v for v in results.values()):
        try:
            results = {
                ...
                "optimized": optimized_ml.get_backtest_results()
                if optimized_ml
                else {},
            }
...
def api_market_data():
    ...
    def _filter_mapping(key: str):
        ...
        if isinstance(mapping, dict):
            return {
                symbol: value for symbol, value in mapping.items() if symbol in active
            }
        return {}
    ...
    return jsonify(
        {
            ...
            "trending_pairs": [
                symbol
                for symbol in dashboard_data.get("trending_pairs", [])
                if symbol in active
            ],
            ...
            "optimized_ensemble_predictions": _filter_mapping(
    ...
    return jsonify(
        {
            ...
            "execution_mode": filters.get(
                "execution_mode", "all" if not default_real_only else "real"
            ),
...
def api_signal_source_performance():
    ...
    for trade in user_trades:
        ...
        perf = signal_performance.setdefault(
            signal_source,
            {
                "total_trades": 0,
                "winning_trades": 0,
        ...
        if pnl > 0:
            ...
            perf["avg_loss"] = (
                ((perf["avg_loss"] * (count - 1)) + abs(pnl)) / count
                if count
                else perf["avg_loss"]
            )
    ...
    for signal_source, perf in signal_performance.items():
        ...
        related_returns = [
            trade.pnl or 0
            for trade in user_trades
            if (trade.signal_source or "unknown") == signal_source
        ]
        if len(related_returns) > 1:
            avg_return = sum(related_returns) / len(related_returns)
            variance = sum((r - avg_return) ** 2 for r in related_returns) / len(
                related_returns
            )
            perf["sharpe_ratio"] = (
                (avg_return / (variance**0.5)) if variance > 0 else 0.0
            )
        else:
            perf["sharpe_ratio"] = 0.0

    sorted_sources = sorted(
        signal_performance.items(), key=lambda item: item[1]["total_pnl"], reverse=True
    )
    return jsonify(
        {
...
def api_confidence_score_analysis():
    ...
    user_trades = UserTrade.query.filter(
        UserTrade.user_id == user_id, UserTrade.confidence_score.isnot(None)
    ).all()
...
def api_user_strategy_performance():
    ...
    for trade in user_trades:
        ...
        perf = strategy_performance.setdefault(
            strategy,
            {
                "total_trades": 0,
                "winning_trades": 0,
    ...
    for strategy, perf in strategy_performance.items():
        ...
        avg_win = (
            perf["total_wins"] / perf["winning_trades"]
            if perf["winning_trades"]
            else 0.0
        )
        ...
        perf["profit_factor"] = (
            perf["total_wins"] / perf["total_losses"]
            if perf["total_losses"] > 0
            else float("inf")
        )
        ...
        if len(strategy_returns) > 1:
            ...
            variance = sum((r - avg_return) ** 2 for r in strategy_returns) / len(
                strategy_returns
            )
            perf["sharpe_ratio"] = (
                (avg_return / (variance**0.5)) if variance > 0 else 0.0
    ...
    for strategy, perf in strategy_performance.items():
        profit_factor = (
            perf["profit_factor"] if perf["profit_factor"] != float("inf") else 10
        )
