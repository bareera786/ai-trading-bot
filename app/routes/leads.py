...
def _notify_ops(lead: Lead) -> None:
    ...
    try:
        ...
        current_app.logger.warning(
            "Failed to notify webhook for lead %s: %s", lead.email, exc
        )
...
def capture_lead():
    ...
    lead = Lead(
        ...
        details={
            ...
            if k not in {"name", "email", "company", "message", "notes", "source"}
        },
    )
    ...
    try:
        ...
        return (
            jsonify({"error": "Unable to save your request. Please try again later."}),
            500,
        )
