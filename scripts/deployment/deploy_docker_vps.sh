...
echo "🐳 Starting Docker-based AI Trading Bot deployment to VPS..."
...
if [ -z "$SSH_PASSWORD" ]; then
    echo "🔑 Please provide SSH password for $VPS_USER@$VPS_HOST:"
    read -s SSH_PASSWORD
    echo ""
fi
...
# Fix permissions on VPS immediately after sync
echo "🔧 Fixing permissions on VPS..."
ssh_cmd "cd $VPS_PATH && chmod +x scripts/fix_permissions.sh && ./scripts/fix_permissions.sh $VPS_USER"
echo "✅ Permissions fixed on VPS"
echo ""
...
CREATE_BOOTSTRAP_USERS=1
ENABLE_RIBS_OPTIMIZATION=1
RIBS_CHECKPOINT_DIR=/app/bot_persistence/ribs_checkpoints
...
# Credentials (Injected from local environment)
...
echo "🐳 Step 3: Ensuring Docker is installed on VPS..."
...
if ! ssh_cmd "command -v docker >/dev/null 2>&1"; then
    ...
    echo "✅ Docker installed on VPS"
else
    echo "✅ Docker already installed on VPS"
fi

# Check if Docker Compose is installed
if ! ssh_cmd "command -v docker-compose >/dev/null 2>&1 || docker compose version >/dev/null 2>&1"; then
    echo "⚠️  Docker Compose not found. Please install Docker Compose manually on VPS:"
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "🔑 Please provide sudo password for Docker Compose installation:"
        ...
        echo "✅ Docker Compose installed on VPS"
    else
        echo "❌ Docker Compose installation skipped. Please install manually and run deployment again."
        exit 1
    fi
else
    echo "✅ Docker Compose already available on VPS"
fi

echo ""

# Step 4: Deploy with Docker
echo "🚀 Step 4: Deploying with Docker on VPS..."
...
if get_yes_no_choice "Do you want to enable monitoring with Prometheus and Grafana? (y/N): " "${ENABLE_MONITORING:-}" "n"; then
    ...
    echo "⏭️  Skipping monitoring setup"
...
echo "  🌐 Application: http://$VPS_HOST:5000"
echo "  🐘 PostgreSQL: $VPS_HOST:5432"
echo "  🔴 Redis: $VPS_HOST:6379"

if [ "$MONITORING_ENABLED" = true ]; then
    echo "  📊 Prometheus: http://$VPS_HOST:9090"
    echo "  📈 Grafana: http://$VPS_HOST:3000 (admin/admin123)"
fi

if [ "$NGINX_ENABLED" = true ]; then
    echo "  🌐 Nginx Proxy: http://$VPS_HOST (port 80)"
fi
...
echo ""
echo "🚀 Your AI Trading Bot is now running in Docker containers on VPS!"
