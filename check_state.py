...
def _get_ai_context(app):
    ...
    if ctx is None:  # pragma: no cover - defensive guard
        raise RuntimeError('AI bot context is unavailable; ensure bootstrap_runtime() ran.')
    return ctx
...
def main() -> None:
    ...
    with app.app_context():
        ...
        print('ultimate_trader.trading_enabled:', getattr(ultimate, 'trading_enabled', None))
        print('optimized_trader.trading_enabled:', getattr(optimized, 'trading_enabled', None))

    state_path = Path('bot_persistence/bot_state.json')
    if state_path.exists():
        with state_path.open('r') as f:
            state = json.load(f)
        print('state file trading_enabled:', state.get('trader_state', {}).get('trading_enabled'))
    else:
