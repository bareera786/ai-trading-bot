...
def _normalize_indicator_profile(raw_profile: Any, ctx: dict[str, Any]) -> Optional[str]:
    if not raw_profile:
        return None
...
def dashboard():
    ...
    version_label = ctx.get('version_label') or ctx.get('ai_bot_version') or 'Ultimate AI Bot'
    response = make_response(render_template('dashboard.html', version_label=version_label))
...
@dashboard_bp.route('/api/indicator_selection', methods=['GET'], endpoint='api_get_indicator_selection')
...
@dashboard_bp.route('/api/indicator_selection', methods=['POST'], endpoint='api_set_indicator_selection')
@login_required
    ...
    try:
        ...
        return jsonify({
            ...
            'system_status': {
                ...
                'real_trading_enabled': getattr(user_trader, 'real_trading_enabled', False),
...
@dashboard_bp.route('/api/dashboard_performance', endpoint='api_dashboard_performance_metrics')
def api_dashboard_performance_metrics():
    ...
    def _performance(trader):
        ...
        fallback = {
            ...
            'system_status': 'active' if getattr(trader, 'trading_enabled', False) else 'inactive',
        }
...
def api_qfm_analytics():
    ...
    for symbol in active_symbols:
        try:
            if ml_system and hasattr(ml_system, 'get_qfm_features') and symbol in getattr(ml_system, 'models', {}):
