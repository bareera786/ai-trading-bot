...
def save_symbol_state() -> None:
    ...
    fd, temp_path = tempfile.mkstemp(
        dir=os.path.dirname(path), prefix="symbols_", suffix=".json"
    )
    try:
...
def get_user_trading_universe(user) -> list[str]:
    """Get trading universe for a specific user, falling back to global if none selected."""
    if user and hasattr(user, 'get_selected_symbols'):
        selected = user.get_selected_symbols()
        if user.is_premium and hasattr(user, 'get_custom_symbols'):
            custom = user.get_custom_symbols()
            selected.extend(custom)
        if selected:
            return selected
    return get_active_trading_universe()


def get_training_universe() -> list[str]:
    """Get union of all user-selected symbols and premium user custom symbols for ML training."""
    from app.extensions import db
    from app.models import User
    try:
        all_selected = set()
    ...
    try:
        ...
        for portfolio_key in ["portfolio", "optimized_portfolio"]:
            ...
            positions = (
                ...
                if isinstance(portfolio, MutableMapping)
                else None
            )
            if isinstance(positions, list):
                portfolio["positions"] = [
                    pos for pos in positions if pos.get("symbol") != normalized
                ]

        trending = dashboard_data.get("trending_pairs")
        if isinstance(trending, list):
            dashboard_data["trending_pairs"] = [
                pair for pair in trending if pair != normalized
            ]
    except Exception:
        pass
