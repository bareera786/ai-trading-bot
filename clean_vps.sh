...
# Try to stop systemd service if we have sudo access
echo "Attempting to stop systemd service (you may need to enter sudo password)..."
echo "Please run these commands on your VPS as root or with sudo:"
echo ""
echo "sudo systemctl stop ai-trading-bot 2>/dev/null || true"
echo "sudo systemctl disable ai-trading-bot 2>/dev/null || true"
echo "sudo rm -f /etc/systemd/system/ai-trading-bot.service"
echo "sudo systemctl daemon-reload"
echo ""

# Kill any running processes we can kill without sudo
$SSH_CMD "pkill -f 'flask run' || true"
$SSH_CMD "pkill -f 'gunicorn' || true"
$SSH_CMD "pkill -f 'python.*wsgi' || true"

echo "✅ Local processes killed (systemd service needs manual sudo commands above)"
echo ""
