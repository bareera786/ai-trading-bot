...
class HealthReportService:
    ...
    def __init__(
        ...
        *,
        ...
        summary_evaluator: Callable[[Mapping[str, Any], Mapping[str, Any]], Dict[str, Any]],
    ...
    def start_periodic_refresh(self) -> threading.Thread:
        ...
        thread = threading.Thread(target=self._refresh_loop, args=(interval,), daemon=True)
        thread.start()
        return thread
    ...
    def _run_health_backtests_if_enabled(self) -> str | None:
        ...
        command = [
            sys.executable,
            script_path,
            '--years', str(self._config.get('backtest_years', 1)),
            '--interval', str(self._config.get('backtest_interval', '1d')),
            '--output', self._config['report_path'],
        ]
    ...
    def _load_health_report_payload(self, path: str) -> Tuple[Dict[str, Any] | None, str | None]:
        ...
        while True:
            try:
                ...
                with self._lock:
                    ...
                    self._dashboard_data['health_report']['last_refresh'] = datetime.utcnow().isoformat()
            time.sleep(interval)


def evaluate_health_payload(payload: Mapping[str, Any], config: Mapping[str, Any]) -> Dict[str, Any]:
    thresholds = {
