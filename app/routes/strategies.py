...
def strategy_performance(strategy_name: str):
    ...
    if performance is None:
        return _jsonify_error(
            f"Performance data not available for {strategy_name}", 404
        )
...
def all_strategies_performance():
    ...
    return jsonify({"performance": performance_data, "timestamp": time.time()})
...
def reset_strategies():
    ...
    return jsonify(
        {"message": "All strategies reset to default state", "timestamp": time.time()}
    )
...
def optimization_status():
    ...
    return jsonify(
        {"optimization_status": status, "timestamp": datetime.now().isoformat()}
    )
