...
class FuturesMarketDataService:
    ...
    def __init__(
        ...
        *,
        ...
        manual_trade_handler: Callable[[str, Mapping[str, Any], Mapping[str, Any], Mapping[str, Any]], None],
    ...
    def start(self) -> bool:
        ...
        self._thread = threading.Thread(target=self._run_loop, name='FuturesMarketDataLoop', daemon=True)
        self._thread.start()
    ...
    def run_once(self) -> None:
        ...
        market_regime = self.dashboard_data.get('system_status', {}).get('market_regime', 'NEUTRAL')
        ...
        for symbol in self.futures_symbols:
            ...
            prediction = self.futures_ml_system.predict_futures(symbol, market_data) or {}
            predictions[symbol] = prediction

            confidence = float(prediction.get('ultimate_ensemble', {}).get('confidence', 0.55) or 0.55)
            signal_name = (prediction.get('ultimate_ensemble', {}).get('signal') or 'HOLD').upper()
            ...
            quantity, margin_required, notional_value = self.futures_ml_system.futures_module.calculate_futures_position_size(
                symbol,
                futures_balance,
                leverage,
                entry_price,
            ...
            if self.futures_ml_system.futures_module.should_avoid_funding_period(symbol):
                funding_alerts.append(f"Avoid funding window for {symbol}")

            self.manual_trade_handler(symbol, market_data, prediction, sizing_map.get(symbol, {}))
        ...
        with self.futures_data_lock:
            ...
            self.futures_dashboard_state['config'] = dict(self.futures_ml_system.futures_module.futures_config)
            ...
            if account_overview:
                ...
                state_portfolio['available_margin'] = float(max(0.0, state_portfolio.get('balance', 0.0) - total_margin))
                state_portfolio.pop('account_overview', None)
            state_portfolio['equity'] = float(state_portfolio['balance'] + state_portfolio.get('unrealized_pnl', 0.0))
    ...
    def _get_interval(self) -> float:
        default_interval = max(10.0, float(self.trading_config.get('futures_update_interval', 30)))
        return max(10.0, float(self.trading_config.get('futures_update_interval', default_interval)))
