...
def _coerce_bool_with_ctx(ctx: dict[str, Any], value: Any, default: bool = True) -> bool:
...
@trading_bp.route('/api/binance/credentials', methods=['GET', 'POST', 'DELETE'], endpoint='api_binance_credentials')
@login_required
def api_binance_credentials():
    ...
    if not all([callable(status_fn), callable(apply_credentials), credentials_store, ultimate_trader, optimized_trader]):
    ...
    if method == 'GET':
        ...
        dashboard_data['system_status']['paper_trading'] = getattr(ultimate_trader, 'paper_trading', True)
        dashboard_data['system_status']['real_trading_ready'] = bool(getattr(ultimate_trader, 'real_trading_enabled', False))
        dashboard_data['optimized_system_status']['paper_trading'] = getattr(optimized_trader, 'paper_trading', True)
        dashboard_data['optimized_system_status']['real_trading_ready'] = bool(getattr(optimized_trader, 'real_trading_enabled', False))
...
def api_binance_logs():
    ...
    return jsonify({'logs': logs, 'count': len(logs), 'limit': limit, 'account_type': account_type or 'all'})
...
def api_futures_dashboard():
    ...
    if not futures_data_lock or (manual_service is None and not callable(ensure_manual_defaults)):
...
@trading_bp.route('/api/futures/manual', methods=['GET'], endpoint='api_futures_manual_state')
def api_futures_manual_state():
    ...
    if manual_service is not None:
        try:
            manual = manual_service.get_manual_state(include_symbols=True, update_dashboard=True)
    ...
    if not all([callable(ensure_manual_defaults), futures_manual_lock, futures_manual_settings]):
        return jsonify({'error': 'Manual futures state unavailable'}), 500
...
@trading_bp.route('/api/futures/manual/select', methods=['POST'], endpoint='api_futures_manual_select')
def api_futures_manual_select():
    ...
    if manual_service is not None:
        try:
            ...
            current_app.logger.error('Failed to update manual futures selection: %s', exc)
            return jsonify({'error': 'Unable to update manual futures settings'}), 500

    dashboard_data = _get_dashboard_data(ctx)

    if not all([callable(ensure_manual_defaults), futures_manual_lock, futures_manual_settings, trading_config]):
...
@trading_bp.route('/api/futures/manual/toggle', methods=['POST'], endpoint='api_futures_manual_toggle_trading')
@login_required
def api_futures_manual_toggle_trading():
    ...
    if not all([callable(ensure_manual_defaults), futures_manual_lock, futures_manual_settings, trading_config, ultimate_trader]):
        return jsonify({'error': 'Manual futures toggle unavailable'}), 500
    ...
    return jsonify({'auto_trade_enabled': enable, 'mode': mode, 'selected_symbol': futures_manual_settings.get('selected_symbol')})
...
def api_spot_toggle():
    ...
    try:
        ...
        return jsonify({'trading_enabled': enable, 'message': f"Spot trading {'enabled' if enable else 'disabled'} for both profiles"})
...
@trading_bp.route('/api/futures/toggle', methods=['POST'], endpoint='api_futures_toggle')
@login_required
def api_futures_toggle():
    ...
    try:
        ...
        if enable is None:
            enable = not bool(getattr(ultimate_trader, 'futures_trading_enabled', False))
        ...
        return jsonify({'futures_trading_enabled': enable, 'message': f"Futures trading {'enabled' if enable else 'disabled'} for both profiles"})
    except Exception as exc:
