...
class ModelTrainingWorker:
    ...
    def start_ultimate_startup_training(self) -> None:
        self._start_thread(
            self.train_ultimate_models_on_startup, name="UltimateModelTraining"
        )

    def start_optimized_startup_training(self) -> None:
        self._start_thread(
            self.train_optimized_models_on_startup, name="OptimizedModelTraining"
        )
    ...
    def train_ultimate_models_on_startup(self) -> None:
        ...
        loaded_active = sum(
            1 for sym in active_symbols if sym in self.ultimate_ml_system.models
        )
        ...
        avg_indicators = self._calculate_average_indicator_usage(
            self.ultimate_ml_system
        )
        status["indicators_used"] = avg_indicators
        self._log(
            f"✅ ULTIMATE models ready! (Avg indicators: {avg_indicators}/{len(self.best_indicators)})"
        )
    ...
    def train_optimized_models_on_startup(self) -> None:
        ...
        loaded_active = sum(
            1 for sym in active_symbols if sym in self.optimized_ml_system.models
        )
        ...
        avg_indicators = self._calculate_average_indicator_usage(
            self.optimized_ml_system
        )
        opt_status["indicators_used"] = avg_indicators
        self._log(
            f"✅ OPTIMIZED models ready! (Avg indicators: {avg_indicators}/{len(self.best_indicators)})"
        )
