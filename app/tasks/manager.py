...
def get_self_improvement_worker() -> Any | None:
    """Get the self-improvement worker from the global manager."""
    if _background_task_manager and _background_task_manager.self_improvement_worker:
        return _background_task_manager.self_improvement_worker
    return None
...
class BackgroundTaskManager:
    ...
    def start_background_tasks(
        ...
        *,
        ...
        if (
            ...
            and self.futures_market_data_service
        ):
            self._start_service(
        ...
        if start_ultimate_training and self.model_training_worker:
            try:
                ...
                self._log(
                    f"⚠️ Failed to dispatch ultimate model training: {exc}",
                    warning=True,
                )
        ...
        if start_optimized_training and self.model_training_worker:
            try:
                ...
                self._log(
                    f"⚠️ Failed to dispatch optimized model training: {exc}",
                    warning=True,
                )
        ...
        if persistence_inputs and self.persistence_scheduler:
            try:
                ...
                self._log(
                    f"⚠️ Failed to start automatic persistence: {exc}", warning=True
                )
    ...
    def start_live_portfolio_updates(self) -> None:
        ...
        try:
            ...
            self._log(
                f"⚠️ Failed to start live portfolio scheduler: {exc}", warning=True
            )
    ...
    def stop_background_tasks(self) -> None:
        ...
        if self.self_improvement_worker:
            try:
                ...
                self._log(
                    f"⚠️ Failed to stop self-improvement worker: {exc}", warning=True
                )
        ...
        if self._persistence_active and self.persistence_scheduler:
            try:
                ...
                self._log(
                    f"⚠️ Failed to stop automatic persistence: {exc}", warning=True
                )
            finally:
                self._persistence_active = False
    ...
    def stop_live_portfolio_updates(self) -> None:
        ...
        try:
            ...
            self._log(
                f"⚠️ Failed to stop live portfolio scheduler: {exc}", warning=True
            )
