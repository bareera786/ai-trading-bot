...
def _import_legacy_module():
	...
	except Exception as exc:  # pragma: no cover - defensive logging for import edge cases
...
def _resolve_binance_credential_methods(module):
	...
	if apply_creds is None or status_getter is None:
		raise RuntimeBuilderError('BinanceCredentialService is missing required helpers')
	return service, apply_creds, status_getter

def _build_payload_from_module(module, *, indicator_runtime: Optional[IndicatorRuntime]) -> dict[str, Any]:
	...
	return build_ai_bot_context_payload(
		...
		get_all_indicator_selections=_require_attr(module, 'get_all_indicator_selections'),
		set_indicator_selection=_require_attr(module, 'set_indicator_selection'),
		refresh_indicator_dashboard_state=_require_attr(module, 'refresh_indicator_dashboard_state'),
		...
		get_active_trading_universe=symbols.get_active_trading_universe,
		...
		ensure_futures_manual_defaults=_require_attr(module, '_ensure_futures_manual_defaults'),
		...
		futures_market_data_service=_require_attr(module, 'futures_market_data_service'),
...
def assemble_runtime_context(*, flask_app=None, force: bool = False) -> Optional[RuntimeContext]:
