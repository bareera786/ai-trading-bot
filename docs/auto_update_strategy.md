...
| Item | Notes |
| --- | --- |
| Deployment target | Ubuntu 22.04 LTS VPS instances running Docker Compose stack |
...
6. **Restart service**
   - For Docker Compose: `docker compose -f docker-compose.prod.yml restart ai-trading-bot`.
...
| Phase 1 | Scheduler integration (cron/APScheduler job) with version tracking. | Requires config entries for update cadence. |
