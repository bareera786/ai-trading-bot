...
class MarketDataService:
    ...
    def __init__(
        ...
        *,
        ...
        self.sleep_interval = max(5.0, float(sleep_interval) if sleep_interval else 30.0)
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run_loop, name='MarketDataServiceLoop', daemon=True)
        self._thread.start()
    ...
    def run_once(self) -> None:
        ...
        self.dashboard_data['trending_pairs'] = [pair for pair in trending_pairs if pair in active_symbols]
        ...
        if self.trading_config.get('parallel_processing'):
            if market_data:
                ml_predictions = self.parallel_engine.parallel_predict(active_symbols, market_data, self.ultimate_ml_system)
                optimized_ml_predictions = self.parallel_engine.parallel_predict(active_symbols, market_data, self.optimized_ml_system)
        else:
            for symbol in active_symbols:
                ...
                opt_pred = self.optimized_ml_system.predict_professional(symbol, snapshot)
                if opt_pred:
                    optimized_ml_predictions[symbol] = opt_pred

        if ml_predictions:
            self.ultimate_ml_system.ensemble_system.create_correlation_matrix(ml_predictions)
        if optimized_ml_predictions:
            self.optimized_ml_system.ensemble_system.create_correlation_matrix(optimized_ml_predictions)

        ensemble_prediction = self.ultimate_ml_system.ensemble_system.get_ensemble_prediction(ml_predictions, market_data)
        optimized_ensemble_prediction = self.optimized_ml_system.ensemble_system.get_ensemble_prediction(optimized_ml_predictions, market_data)
        self.dashboard_data['ensemble_predictions'] = ensemble_prediction or {}
        self.dashboard_data['optimized_ensemble_predictions'] = optimized_ensemble_prediction or {}
        ...
        for symbol in active_symbols:
            ...
            if ultimate_qfm_engine and market_snapshot:
                try:
                    ...
                    if qfm_signal:
                        qfm_signals[symbol] = self._format_qfm_signal(qfm_signal, market_snapshot, symbol)
                except Exception as exc:
                    self.bot_logger.warning("Ultimate QFM update failed for %s: %s", symbol, exc)
            if optimized_qfm_engine and market_snapshot:
                try:
                    ...
                    if opt_signal:
                        optimized_qfm_signals[symbol] = self._format_qfm_signal(opt_signal, market_snapshot, symbol)
                except Exception as exc:
                    self.bot_logger.warning("Optimized QFM update failed for %s: %s", symbol, exc)

            if market_snapshot and len(history) >= 20:
                crt_signal = self.ultimate_ml_system.generate_crt_signals(symbol, market_snapshot, history)
                crt_signals[symbol] = crt_signal
                optimized_crt_signal = self.optimized_ml_system.generate_crt_signals(symbol, market_snapshot, history)
                ...
                ai_signals[symbol] = self._build_ai_signal(self.ultimate_trader, ml_predictions.get(symbol), ensemble_prediction, success, message, crt_signal)
        ...
        current_prices = {symbol: data['price'] for symbol, data in market_data.items() if 'price' in data}
        ...
        optimized_portfolio = self.optimized_trader.get_portfolio_summary(current_prices)
        self.dashboard_data['portfolio'] = portfolio
        self.dashboard_data['optimized_portfolio'] = optimized_portfolio
        self.dashboard_data['trade_statistics'] = self.ultimate_trader.get_trade_statistics()
        self.dashboard_data['optimized_trade_statistics'] = self.optimized_trader.get_trade_statistics()

        self._update_system_status(portfolio)
        self._update_optimized_status(optimized_portfolio)

        self.dashboard_data['safety_status'] = self.ultimate_trader.safety_manager.get_status_snapshot()
        self.dashboard_data['optimized_safety_status'] = self.optimized_trader.safety_manager.get_status_snapshot()
        self.dashboard_data['real_trading_status'] = self.ultimate_trader.get_real_trading_status()
        self.dashboard_data['optimized_real_trading_status'] = self.optimized_trader.get_real_trading_status()
        self.dashboard_data['binance_credentials'] = self.binance_credential_service.get_status(include_connection=True)
        ...
        if hasattr(self.optimized_trader.trade_history, 'get_journal_events'):
            journal_events.extend(
                ...
                for event in self.optimized_trader.trade_history.get_journal_events(limit=50)
            )
        ...
        self._log_status_summary(active_symbols, ai_signals, optimized_ai_signals, portfolio, optimized_portfolio, crt_signals, optimized_crt_signals)

    def update_performance_metrics(self) -> None:
        try:
            performance = self.ultimate_trader.trade_history.get_trade_statistics()['summary']
            self.dashboard_data['performance'] = performance
            optimized_performance = self.optimized_trader.trade_history.get_trade_statistics()['summary']
            ...
            if 'bot_efficiency' in portfolio:
                self.dashboard_data['system_status']['bot_efficiency'] = portfolio['bot_efficiency']['success_rate']
                self.dashboard_data['system_status']['learning_cycles'] = portfolio['bot_efficiency']['learning_cycles']
            if 'bot_efficiency' in optimized_portfolio:
                self.dashboard_data['optimized_system_status']['bot_efficiency'] = optimized_portfolio['bot_efficiency']['success_rate']
                self.dashboard_data['optimized_system_status']['learning_cycles'] = optimized_portfolio['bot_efficiency']['learning_cycles']

            if self.ultimate_ml_system.models:
                indicators, count = 0, 0
                for model in self.ultimate_ml_system.models.values():
                    indicators += model.get('feature_count', len(model.get('feature_cols', [])))
                    count += 1
                self.dashboard_data['system_status']['indicators_used'] = indicators // count if count else 0
            if self.optimized_ml_system.models:
                indicators, count = 0, 0
                for model in self.optimized_ml_system.models.values():
                    indicators += model.get('feature_count', len(model.get('feature_cols', [])))
                    count += 1
                self.dashboard_data['optimized_system_status']['indicators_used'] = indicators // count if count else 0
                self.dashboard_data['optimized_system_status']['models_loaded'] = True

            self.dashboard_data['ml_telemetry']['ultimate'] = self.ultimate_ml_system.get_ml_telemetry()
            self.dashboard_data['ml_telemetry']['optimized'] = self.optimized_ml_system.get_ml_telemetry()
            self.dashboard_data['system_status']['ensemble_active'] = bool(self.ultimate_ml_system.ensemble_system.correlation_matrix)
            self.dashboard_data['optimized_system_status']['ensemble_active'] = bool(self.optimized_ml_system.ensemble_system.correlation_matrix)
        except Exception as exc:
    ...
    def _format_qfm_signal(self, signal: dict[str, Any], snapshot: dict[str, Any], symbol: str) -> dict[str, Any]:
        return {
            'symbol': symbol,
    ...
    def _build_ai_signal(self, trader, prediction, ensemble_prediction, success, message, crt_signal):
        signal_block = (prediction or {}).get(getattr(trader, 'indicator_block_key', 'ultimate_ensemble'), {})
        return {
    ...
    def _update_system_status(self, portfolio: dict[str, Any]) -> None:
        ...
        status['risk_adjustment'] = self.ultimate_trader.risk_manager.get_risk_multiplier()
        status['market_stress'] = self.ultimate_trader.risk_manager.market_stress_indicator
        ...
        status['futures_trading_ready'] = bool(getattr(self.ultimate_trader, 'futures_trading_enabled', False))
        status['futures_manual_auto_trade'] = self.futures_manual_settings.get('auto_trade_enabled', False)
        if 'bot_efficiency' in portfolio:
    ...
    def _update_optimized_status(self, portfolio: dict[str, Any]) -> None:
        ...
        status['risk_adjustment'] = self.optimized_trader.risk_manager.get_risk_multiplier()
        status['market_stress'] = self.optimized_trader.risk_manager.market_stress_indicator
    ...
    def _update_portfolio_efficiency(self, ultimate_portfolio: dict[str, Any], optimized_portfolio: dict[str, Any]) -> None:
        if 'bot_efficiency' in ultimate_portfolio:
            self.dashboard_data['system_status']['bot_efficiency'] = ultimate_portfolio['bot_efficiency']['success_rate']
            self.dashboard_data['system_status']['learning_cycles'] = ultimate_portfolio['bot_efficiency']['learning_cycles']
        if 'bot_efficiency' in optimized_portfolio:
            self.dashboard_data['optimized_system_status']['bot_efficiency'] = optimized_portfolio['bot_efficiency']['success_rate']
            self.dashboard_data['optimized_system_status']['learning_cycles'] = optimized_portfolio['bot_efficiency']['learning_cycles']

    def _log_status_summary(self, active_symbols, ai_signals, optimized_ai_signals, portfolio, optimized_portfolio, crt_signals, optimized_crt_signals):
        ...
        opt_active_count = len([s for s in optimized_ai_signals.values() if s.get('action_taken')])
