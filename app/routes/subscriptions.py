...
from app.models import PaymentSettings, SubscriptionPlan, User
...
def _build_public_plan_payload() -> dict[str, object]:
    ...
    featured_plan = next(
        (plan for plan in plans if plan.is_featured), plans[0] if plans else None
    )
    return {
...
def api_list_subscription_plans():
    try:
        ...
        return jsonify(
            {"plans": [plan.to_dict() for plan in plans], "count": len(plans)}
        )
    except Exception as exc:
    ...
    try:
        ...
        if plan_type not in ALLOWED_SUBSCRIPTION_PLAN_TYPES:
            return (
                jsonify(
                    {
                        "error": f"plan_type must be one of {sorted(ALLOWED_SUBSCRIPTION_PLAN_TYPES)}"
                    }
                ),
                400,
            )
        ...
        if existing_code:
            return (
                jsonify({"error": "A subscription plan with this code already exists"}),
                400,
            )
...
def api_update_subscription_plan(plan_id):
    try:
        ...
        if "duration_days" in data:
            ...
            if duration_days <= 0:
                return (
                    jsonify({"error": "duration_days must be greater than zero"}),
                    400,
                )
...
@subscription_bp.route(
    "/admin/subscription/plans/<int:plan_id>/toggle", methods=["PATCH"]
)
@admin_required
...
def api_assign_user_subscription(username):
    try:
        ...
        auto_renew = coerce_bool(
            data.get("auto_renew"), default=(plan.plan_type != "trial")
        )
        ...
        return jsonify(
            {"success": True, "subscription": serialize_subscription(subscription)}
        )
    except ValueError as ve:
        db.session.rollback()
        return jsonify({"error": str(ve)}), 400
...
def api_cancel_user_subscription(username):
    try:
        ...
        subscription = cancel_user_subscription(
            user, immediate=immediate, reason=reason
        )
        if not subscription:
...
@subscription_bp.route("/admin/payment-settings", methods=["GET"])
@admin_required
def api_get_payment_settings():
    try:
        ...
        if not settings:
            ...
            db.session.commit()
        return jsonify({"payment_address": settings.payment_address or ""})
    except Exception as exc:
        db.session.rollback()
        print(f"Error in GET /api/admin/payment-settings: {exc}")
        return jsonify({"error": "Unable to load payment settings"}), 500


@subscription_bp.route("/admin/payment-settings", methods=["PUT"])
@admin_required
def api_update_payment_settings():
    try:
        data = request.get_json() or {}
        payment_address = (data.get("payment_address") or "").strip()

        if len(payment_address) > 255:
            return jsonify({"error": "payment_address is too long"}), 400
        ...
        settings.payment_address = payment_address or None
        db.session.commit()
        return jsonify(
            {
                "success": True,
                "payment_address": settings.payment_address or "",
            }
        )
    except Exception as exc:
        db.session.rollback()
        print(f"Error in PUT /api/admin/payment-settings: {exc}")
        return jsonify({"error": "Unable to update payment settings"}), 500
