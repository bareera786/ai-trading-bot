...
DEFAULT_COMPOSE_FILE = os.getenv("COMPOSE_FILE", "docker-compose.prod.yml")
DEFAULT_SERVICE = os.getenv("DOCKER_SERVICE", "ai-trading-bot")
DEFAULT_CONTAINER = os.getenv("CONTAINER_NAME", "ai-trading-bot-prod")
...
def restart_service(service: str, compose_file: str) -> None:
    run(["docker", "compose", "-f", compose_file, "restart", service], capture=False)


def tail_logs(container_name: str, lines: int = 100) -> None:
    cmd = ["docker", "logs", container_name, "--tail", str(lines), "-f"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--restart", action="store_true", help="Restart Docker service after update"
    )
    parser.add_argument(
        "--service-name",
        default=DEFAULT_SERVICE,
        help="Docker compose service name to restart",
    )
    parser.add_argument(
        "--container-name",
        default=DEFAULT_CONTAINER,
        help="Container name to tail logs from",
    )
    parser.add_argument(
        "--compose-file",
    ...
    parser.add_argument(
        "--tail", action="store_true", help="Tail container logs after restart"
    )
    parser.add_argument(
        "--allow",
        action="append",
    ...
    if args.restart:
        print(f"🔁 Restarting docker service {args.service_name}...")
        restart_service(args.service_name, args.compose_file)
        if args.tail:
            print("📜 Tailing container logs (Ctrl+C to exit)...")
            try:
                tail_logs(args.container_name)
