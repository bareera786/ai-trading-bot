...
def api_portfolio_live_pnl(user_id: int):
    ...
    for position in positions:
        try:
            ...
            pnl_percent = position.pnl_percent or ((pnl / (avg_price * quantity)) * 100 if avg_price and quantity else 0)
...
def api_portfolio_update():
    ...
    try:
        ...
        if not position:
            ...
            for field in (
                'quantity', 'avg_price', 'current_price', 'max_position_size',
                'stop_loss', 'take_profit', 'auto_trade_enabled', 'risk_level',
            ):
                if field in data:
            ...
            if position.quantity and position.avg_price:
                ...
                position.pnl_percent = (position.pnl / cost_basis) * 100 if cost_basis else 0
...
def api_portfolio_risk(user_id: int):
    ...
    try:
        ...
        for position in positions:
            current_value = (position.quantity or 0) * (position.current_price or position.avg_price or 0)
    ...
    try:
        ...
        if request.method == 'GET':
            risk_settings = {
                ...
                'stop_loss_enabled': portfolio.stop_loss_enabled if portfolio.stop_loss_enabled is not None else True,
                'take_profit_enabled': portfolio.take_profit_enabled if portfolio.take_profit_enabled is not None else True,
                'auto_hedging': portfolio.auto_hedging if portfolio.auto_hedging is not None else False,
                'max_open_positions': portfolio.max_open_positions or 5,
                'preferred_symbols': portfolio.preferred_symbols or [],
                'trading_enabled': portfolio.trading_enabled if portfolio.trading_enabled is not None else True,
            }

            return jsonify({'user_id': user_id, 'risk_settings': risk_settings, 'timestamp': time.time()})
...
def api_user_dashboard(user_id: int):
    ...
    try:
        ...
        concentration = (max((pos['position_value'] for pos in positions_data), default=0) / total_exposure * 100) if total_exposure else 0

        if concentration > 50 or any(pos['pnl_percent'] < -20 for pos in positions_data):
            risk_level = 'high'
        elif concentration > 25 or any(pos['pnl_percent'] < -10 for pos in positions_data):
            risk_level = 'medium'
        else:
            risk_level = 'low'
        ...
        risk_metrics = {
            ...
            'max_drawdown': min((pos['pnl_percent'] for pos in positions_data), default=0),
        }
        ...
        total_pnl_sum = sum((trade.pnl or 0) for trade in UserTrade.query.filter_by(user_id=user_id).all())
