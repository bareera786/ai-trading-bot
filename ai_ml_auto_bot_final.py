...
app.config.setdefault('SOCKETIO_CORS_ALLOWED_ORIGINS', os.getenv('SOCKETIO_CORS_ALLOWED_ORIGINS', '*'))
app.config.setdefault('SOCKETIO_ASYNC_MODE', os.getenv('SOCKETIO_ASYNC_MODE', 'threading'))
...
class UltimateMLTrainingSystem:
    def __init__(self, models_dir: Optional[str] = None, profile_key='ultimate'):
        if models_dir is None:
            resolved_dir = resolve_profile_path('ultimate_models', allow_legacy=False, migrate_legacy=True)
    ...
    def create_ultimate_features(self, df):
        ...
        try:
            ...
            self.log_training("SYSTEM", f"🛠️ Creating {indicator_count} core technical indicators...", 70)
        ...
        try:
            ...
            for col in feature_cols:
                try:
                    ...
                    self.log_training(symbol, f"⚠️ Skipping non-numeric feature: {col}", 0)
        ...
        try:
            ...
            if os.path.exists(metrics_file):
                try:
                    ...
                    self.log_training(symbol, f"⚠️ Metrics store corrupted; moved to {backup_name}", 0)
                    all_metrics = []
    ...
    def ensure_model_ready(self, symbol):
        ...
        with lock:
            ...
            self.log_training(symbol, "⚠️ No saved model detected, generating fallback model...", 0)
...
class OptimizedMLTrainingSystem(UltimateMLTrainingSystem):
    def __init__(self, models_dir: Optional[str] = None):
        if models_dir is None:
            models_dir = resolve_profile_path('optimized_models', allow_legacy=False, migrate_legacy=True)
        elif not os.path.isabs(models_dir):
...
class FuturesMLTrainingSystem(UltimateMLTrainingSystem):
    ...
    def __init__(self, models_dir: Optional[str] = None):
        if models_dir is None:
            models_dir = resolve_profile_path('futures_models', allow_legacy=False, migrate_legacy=True)
        elif not os.path.isabs(models_dir):
            models_dir = resolve_profile_path(models_dir, allow_legacy=True)
...
class UltimateAIAutoTrader:
    ...
    def enable_real_trading(self, api_key=None, api_secret=None, testnet=True, force=False):
        ...
        if not final_hammer and not force:
            try:
                ...
                print("Blocked enable_real_trading: FINAL_HAMMER not set and force not provided.")
            return False
    ...
    def enable_futures_trading(self, api_key=None, api_secret=None, testnet=True, force=False):
        ...
        if not final_hammer and not force:
            try:
                ...
                print("Blocked enable_futures_trading: FINAL_HAMMER not set and force not provided.")
            return False
    ...
    def force_close_all_positions(self, reason='EMERGENCY', current_prices=None):
        ...
        for symbol, position in list(self.positions.items()):
            ...
            closed.append(f"⚠️ Emergency exit {symbol}: {quantity:.4f} @ ${sale_price:.2f} (P&L: {pnl_percent:+.2f}%)")
            del self.positions[symbol]
...
trading_runtime = build_trading_runtime_services(
    ...
    trade_history_factory=lambda: ComprehensiveTradeHistory(log_callback=log_component_event),
...
def register_ai_bot_context(flask_app=None, force=False):
    ...
    if flask_app is None:
        raise RuntimeError('Flask application instance is required to register ai_bot_context')
