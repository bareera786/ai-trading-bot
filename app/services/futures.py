...
class FuturesManualService:
    ...
    def __init__(
        ...
        *,
        ...
        self.settings: Dict[str, Any] = {
            ...
            "auto_trade_enabled": trading_config.get(
                "futures_manual_auto_trade", False
            ),
            "leverage": trading_config.get(
                "futures_manual_leverage",
                trading_config.get("futures_default_leverage", 3),
            ),
            "order_size_usdt": trading_config.get(
    ...
    def get_manual_state(
        self, *, include_symbols: bool = True, update_dashboard: bool = True
    ) -> Dict[str, Any]:
    ...
    def select_symbol(
        ...
        *,
        ...
        if cleaned_symbol not in available:
            raise ValueError(
                f"Symbol {cleaned_symbol} is not in the allowed futures list"
            )
    ...
    def toggle_auto_trading(
        ...
        *,
        ...
        if desired_enable and not getattr(
            ultimate_trader, "futures_trading_enabled", False
        ):
            with self.lock:
    ...
    def handle_manual_trading(
        ...
        if current_side != "FLAT" and (
            target_side == "FLAT" or current_side != target_side
        ):
