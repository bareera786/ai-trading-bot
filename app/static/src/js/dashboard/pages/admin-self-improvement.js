...
function initAdminSelfImprovement() {
...
function updatePerformanceTable(siData) {
    ...
    if (trends.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="6" class="loading-row">No performance data available</td></tr>';
        return;
    }
...
export { initAdminSelfImprovement };

// Auto-initialize when admin-self-improvement page is shown
window.addEventListener('dashboard:admin-self-improvement-visible', () => {
    initAdminSelfImprovement();
}, { once: true });